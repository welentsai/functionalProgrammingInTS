...
const solution1_1 = (
  ...
  pipe(input, RA.map(parseInteger), RA.sequence(O.Applicative))
console.log('map and sequence --> ', solution1_1(['1', '2', '3']))

const solustion2 = (
  input: ReadonlyArray<string>
): O.Option<readonly number[]> =>
  pipe(input, RA.traverse(O.Applicative)(parseInteger))
...
const solution3 = (
  ...
  return pipe(input, RA.filterMap(parseInteger), O.fromNullable)
}
