...
const validateConnectionString2 = (connectionString: string): E.Either<string[], string> => {
    console.log(Date.now(), "-validateConnectionString-")
    T.delay(1000)
    return E.right(connectionString)
}
...
assert.deepStrictEqual(validateAppConfig(data), E.left(new Error("port not valid !!"))) // <= first error
...
// get all errors
const validateAppConfig2 = (input: AppConfig): E.Either<readonly string[], AppConfig> =>
    pipe(
        E.Do,
